...
public class DataGroupLayer extends Layer {
   ...
   public void addData(SGTData data, Attribute attr, DataKey key) throws DataTargetMismatchException {
      ...
      if (rend == null) {  // first data on DataGroupLayer
         ...
         dataLayers_.add(ly);
         if (clipping_) {
            cg.setClip(clipDomain_.getXRange(), clipDomain_.getYRange());
         }
         graph = cg;
      }
...
//    PanelHolder ph = dg_.getPanelHolder();
      ...
      while (stillLooking) {
         ...
         if (tType != DataGroup.REFERENCE) {
            ...
            if (dir == DataGroup.X_DIR) {
               ...
               if (graph == null) {
                  return null;  // graph and transform not yet created
               }
   ...
   public void setClipping(boolean clip) {
      ...
      if (Page.DEBUG) {
         System.out.println("DataGroupLayer: " + getId() + ": clip = " + clip);
      }
      clipping_ = clip;
   ...
   void resetZoom(int x, int y) {
      ...
      if (Page.DEBUG) {
         System.out.println("DataGroupLayer: " + getId() + ": " + x + ", " + y);
      }
      inZoom_ = false;
      xZoomRange_ = null;
      yZoomRange_ = null;
