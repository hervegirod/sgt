...
public class CLIndexedColorMap extends ColorMap
   ...
   public boolean equals(ColorMap cm) {
      ...
      if (cl_ == null || !cl_.equals(((CLIndexedColorMap) cm).cl_)) {
         return false;
      }
      if (colors_.length != ((CLIndexedColorMap) cm).colors_.length) {
         return false;
      }
      for (int i = 0; i < colors_.length; i++) {
         if (!colors_[i].equals(((CLIndexedColorMap) cm).colors_[i])) {
            return false;
         }
      }
      return true;
