...
public class IndexedColorMap extends ColorMap
   ...
   public void setTransform(Transform trans) {
      if (!trans.equals(iTrans_)) {
         ...
         if (iTrans_ != null) {
            iTrans_.removePropertyChangeListener(this);
         }
         iTrans_ = trans;
         iTrans_.setRangeP(0.0, (double) colors_.length);
   ...
   public boolean equals(ColorMap cm) {
      ...
      if (!iTrans_.equals(((IndexedColorMap) cm).iTrans_)) {
         return false;
      }
      if (colors_.length != ((IndexedColorMap) cm).colors_.length) {
         return false;
      }
      for (int i = 0; i < colors_.length; i++) {
         if (!colors_[i].equals(((IndexedColorMap) cm).colors_[i])) {
            return false;
         }
      }
      return true;
