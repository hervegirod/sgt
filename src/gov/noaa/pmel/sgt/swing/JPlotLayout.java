...
public class JPlotLayout extends JGraphicLayout
   ...
   public JPlotLayout(boolean isGrid, boolean isXTime, boolean isYTime, String id, Image img, boolean is_key_pane) {
      this(isGrid ? GRID : LINE, isXTime, isYTime, id, img, is_key_pane);
   }
   ...
   public JPlotLayout(boolean isGrid, boolean isPoints, boolean isXTime, boolean isYTime, String id, Image img, boolean is_key_pane) {
      this(isGrid ? GRID : (isPoints ? POINTS : LINE), isXTime, isYTime, id, img, is_key_pane);
   }
...
//        colorKey_.setBorderStyle(ColorKey.PLAIN_LINE);
    ...
    * attributes and description.The description will be taken from the dependent variable name
    *
    * @param lines the lines
    */
   public void addData(Collection lines) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(Collection)");
      }
   ...
   public void addData(Collection lines, Attribute attr) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(Collection, Attribute)");
      }
   ...
   public void addData(PointCollection points, String descrip) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(PointCollection, String)");
      }
   ...
   public void addData(Collection lines, String descrip) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(Collection, String)");
      }
   ...
   public void addData(Collection lines, Attribute attr, String descrip) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(Collection, Attribute, String)");
      }
   ...
   public void addData(SGTData datum) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(SGTData)");
      }
   ...
   public void addData(SGTData datum, Attribute attr) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(SGTData, Attribute)");
      }
      addData(datum, attr, null);
   }

   /**
    * Add data to the layout.  <code>JPlotLayout</code> will use a default attribute.
    *
    * @param datum data to be added
   ...
   public void addData(SGTData datum, String descrip) {
      if (Debug.DEBUG) {
         System.out.println("JPlotLayout.addData(SGTData, String)");
      }
    ...
    * <code>Z_AXIS</code> will be assigned to Y axis. If this is not the first invocation of <code>addData</code> a new
         ...
         if (isOverlayed()) {
            ...
            if (plotType_ == POINTS) {
               ...
               if (attr != null && attr instanceof LineAttribute) {
                  ...
                  if (Debug.DEBUG) {
                     System.out.println("Create new LineAttribute");
                  }
                  if (len >= 2) {
                     lineAttr = new LineAttribute(LineAttribute.SOLID,
                        markList_[layerCount_ % 8],
   ...
   public void setRangeNoVeto(Domain domain) {
      ...
      //
      if (Debug.DEBUG) {
         System.out.println("setRangeNoVeto: " + domain.toString());
      }
      boolean batch = isBatch();
   ...
   void setXRange(Range2D rnge) {
      ...
      LinearTransform xt = (LinearTransform) graph.getXTransform();
      if (Debug.DEBUG) {
         System.out.println("setXRange start, end, delta = "
...
//    warnHeight_ = second;
   ...
   public void propertyChange(PropertyChangeEvent evt) {
      ...
      if (evt.getSource() instanceof Attribute) {
         ...
         if (Debug.EVENT) {
            System.out.println("JPlotLayout: Attribute change: " + evt.getPropertyName());
         }
   ...
   public void init() {
      if (Debug.DEBUG) {
         System.out.println("JPLotLayout: init()");
      }
      if (computeScroll_) {
